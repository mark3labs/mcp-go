...
func TestOAuthHandler_ProductionCachedClientScenario(t *testing.T) {
	// This test simulates the production issue where a cached MCP client
	// may retain a stale or canceled context, causing mysterious "context canceled" errors
	...
	t.Run("Token refresh with canceled context during refresh", func(t *testing.T) {
		...
		// This simulates the exact timing issue mentioned in the production problem
		tokenStore := NewMemoryTokenStore()
		...
		// Should get context.Canceled error, not a mysterious database error
		if !errors.Is(err, context.Canceled) {
			t.Errorf("Expected context.Canceled error during refresh, got %v", err)
		}

		// Verify the error message is clear and not misleading
		if strings.Contains(err.Error(), "database") || strings.Contains(err.Error(), "connection refused") {
			t.Errorf("Error message should not mention database issues, got: %v", err)
		}
	})
}
