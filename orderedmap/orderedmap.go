...
func NewFromMap(m map[string]interface{}) *OrderedMap {
	jsonData, err := json.Marshal(m)
	if err != nil {
		panic("Failed to marshal map to JSON: " + err.Error())
	}
	o := New()
	if err := json.Unmarshal(jsonData, &o.values); err != nil {
		panic("Failed to unmarshal JSON to OrderedMap: " + err.Error())
	}
	return o
}
