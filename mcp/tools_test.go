	...
	tests := []struct {
		...
		},
		{
			...
			newTool: NewTool("new-enum-array",
				...
				WithArray("status",
					Description("Filter by status"),
					ItemsStringEnum([]string{"active", "inactive", "pending"}),
				),
			),
		},
		{
			name: "ItemsString with options",
			...
			newTool: NewTool("new-string-with-opts",
				...
				WithArray("names",
					Description("List of names"),
					ItemsString(MinLength(1), MaxLength(50)),
				),
			),
		},
		{
			name: "ItemsNumber basic",
		...
		{
			name: "ItemsNumber with constraints",
			...
			newTool: NewTool("new-number-with-constraints",
				...
				WithArray("ratings",
					Description("List of ratings"),
					ItemsNumber(Min(0), Max(10)),
				),
			),
		},
		{
			name: "ItemsBoolean basic",
