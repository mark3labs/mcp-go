...
Reject unauthenticated requests before they reach the tool handler:
...
Middleware registered via `Use()` also applies to tools that run as tasks (via `executeRegularToolAsTask`). You do not need separate configuration for task-routed tool calls — the same middleware chain is applied automatically.
...
// This middleware applies to both regular and task-routed calls
s.Use(loggingMiddleware)
```
