...
func TestPromptGeneration(t *testing.T) {
    ...
    assert.Contains(t, result.Messages[0].Content.Text, "best practices")
}

func TestPromptWithResources(t *testing.T) {
    // Mock resource fetching
    mockFetchResource := func(ctx context.Context, uri string) (ResourceData, error) {
        return ResourceData{
            URI:     uri,
    ...
    req := mcp.GetPromptRequest{
        Params: mcp.GetPromptRequestParams{
            ...
            Arguments: map[string]interface{}{
                "document_uri":   "docs://sample",
                "analysis_type": "summary",
            },
        },
    }

    result, err := handleResourceEmbeddedPrompt(context.Background(), req)
    
    assert.NoError(t, err)
    assert.Contains(t, result.Messages[0].Content.Text, "Sample document content")
}
```
...
- Handle resource fetch failures gracefully
- Validate resource content before embedding
- Consider resource size limits

### Performance
- Minimize resource fetching in prompts
- Use efficient string building for large prompts
- Consider prompt caching for expensive operations
- Implement timeouts for resource access
