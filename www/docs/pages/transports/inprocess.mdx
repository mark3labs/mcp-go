...
func TestMCPTools(t *testing.T) {
    ...
    s.AddTool(
        mcp.NewTool("test_tool",
            mcp.WithDescription("Tool for testing"),
...
func TestMCPResources(t *testing.T) {
    ...
    require.NoError(t, err)
    assert.Equal(t, "test://123", result.Contents[0].URI)
}
```
...
func TestApplicationIntegration(t *testing.T) {
    ...
    // Test application functionality through MCP
    result, err := app.ProcessWithMCP(ctx, map[string]interface{}{
        "type": "test_data",
    ...
    status, err := app.mcpClient.CallTool(ctx, mcp.CallToolRequest{
        Params: mcp.CallToolRequestParams{
            Name: "get_app_status",
        },
    })
    ...
    require.NoError(t, err)
    assert.Contains(t, status.Content[0].Text, "uptime")
}
```
...
- **Use object pools**: Pool expensive objects
- **Avoid unnecessary copying**: Pass data by reference when possible
- **Profile regularly**: Use Go's profiling tools to identify bottlenecks

### Memory Management
- **Implement proper cleanup**: Close resources and clear references
- **Use weak references**: Avoid memory leaks in long-running applications
- **Monitor memory usage**: Track allocations and GC pressure
- **Pool large objects**: Reuse expensive allocations

### Error Handling
- **Handle panics**: Use recovery mechanisms
- **Validate inputs**: Check all parameters thoroughly
- **Provide context**: Include helpful error messages
- **Log appropriately**: Balance detail with performance

### Testing
- **Test in isolation**: Use in-process transport for unit tests
- **Mock dependencies**: Isolate components under test
- **Benchmark performance**: Measure latency and throughput
- **Test error conditions**: Verify error handling paths
