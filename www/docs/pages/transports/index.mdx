...
- **[HTTP](/transports/http)** - Traditional HTTP for REST-like interactions
...
| **HTTP** | Web services, APIs | Standard protocol, caching, load balancing | No real-time, more complex |
...
func startServer(s *server.MCPServer) error {
    ...
    case "sse":
        return server.ServeSSE(s, getPort())
    case "http":
        return server.ServeHTTP(s, getPort())
    case "inprocess":
...
func main() {
    ...
    if err := server.ServeHTTP(s, ":8081"); err != nil {
...
- **[HTTP Transport](/transports/http)** - Traditional web services
