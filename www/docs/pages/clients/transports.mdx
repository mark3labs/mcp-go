...
func createStdioClient() {
    ...
    // Initialize connection with proper request
...
type ManagedStdioClient struct {
    ...
    options     transport.StdioOptions
...
func NewManagedStdioClient(options transport.StdioOptions) (*ManagedStdioClient, error) {
...
func (msc *ManagedStdioClient) start() error {
    ...
    if err := stdioTransport.Start(msc.ctx); err != nil {
        return err
    }
    
    client := client.NewClient(stdioTransport)
    
    // Initialize with proper request
    initRequest := mcp.InitializeRequest{}
    initRequest.Params.ProtocolVersion = mcp.LATEST_PROTOCOL_VERSION
    initRequest.Params.ClientInfo = mcp.Implementation{
        Name:    "Managed Stdio Client",
        Version: "1.0.0",
    }
    initRequest.Params.Capabilities = mcp.ClientCapabilities{}
    
    if _, err := client.Initialize(msc.ctx, initRequest); err != nil {
        client.Close()
        return err
    }
...
func createCustomStreamableHTTPClient() {
    ...
    c, err := client.NewStreamableHttpClient("https://api.example.com/mcp",
...
func createStreamableHTTPClientWithSession() {
    // Create StreamableHTTP client with options
    sessionID := "existing-session-id" // fetch existing session ID 
    c, err := client.NewStreamableHttpClient("https://api.example.com/mcp",
    ...
    if errors.Is(err, transport.ErrSessionTerminated) { 
        ...
        serverInfo, err := c.Initialize(ctx, initRequest) // Reinitialize if session is terminated
        if err != nil {
            log.Fatal(err)
        }
        
        // Get the transport to access session ID
        trans := c.GetTransport()
        stdioTrans, ok := trans.(*transport.StreamableHTTP)
        if ok {
            sessionID = stdioTrans.GetSessionId() // Update session ID
        }
    }
}
```
...
func createSSEClient() {
    ...
    notificationChan := make(chan mcp.JSONRPCNotification)
