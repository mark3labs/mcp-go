...
To simplify connection to popular providers that may not support standard discovery, `mcp-go` includes built-in presets.

**Available Presets:**
- `client.GitHubProvider`: Pre-configured for GitHub's OAuth flow (skips discovery, disables dynamic registration).
...
oauthConfig := client.OAuthConfig{
    ...
    PKCEEnabled:  false,
    // Use the built-in GitHub provider preset
